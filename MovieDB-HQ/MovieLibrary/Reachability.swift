...
private let reachabilityProperty: MutableProperty<Reachability?> = {
  ...
  let currentValue = reachability(forFlags: reachabilityFlags)
  if currentValue != .none {
    return MutableProperty(nil)
  } else {
    return MutableProperty(currentValue)
  }
}()

private let queue = DispatchQueue(label: "com.HQ.reachability", attributes: [])
